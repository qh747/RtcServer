...
func ActErrNotfound(w http.ResponseWriter, r *http.Request) {
	...
	Log.Logger.Errorf("Action not found error. request: %s", DumpAction(r))
	err.Act(w, r)
}
...
func ActErrInternalError(w http.ResponseWriter, r *http.Request) {
	...
	Log.Logger.Errorf("Action server internal error. request: %s", DumpAction(r))
	err.Act(w, r)
}
...
func ActErrInvalidRequest(w http.ResponseWriter, r *http.Request, msg string) {
	...
	Log.Logger.Errorf("Action invalid request error. request: %s", DumpAction(r))
	err.Act(w, r)
}
...
func ActErrOther(w http.ResponseWriter, r *http.Request, code int, info string) {
	...
	Log.Logger.Errorf("Action other error. code: %d, info: %s, request: %s", code, info, DumpAction(r))
	err.Act(w, r)
}
